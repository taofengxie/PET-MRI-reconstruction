...
class GaussianFourierProjection(nn.Module):
    ...
    def __init__(self, embedding_size=256, scale=1.0):
        ...
        self.W = nn.Parameter(torch.randn(embedding_size)
                              * scale, requires_grad=False) # 将不可训练的tensor参数转化为可训练的parameter参数
...
class ResnetBlockBigGANpp(nn.Module):
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, up=False, down=False,
        ...
        out_ch = out_ch if out_ch else in_ch
        self.GroupNorm_0 = nn.GroupNorm(num_groups=min(
            in_ch // 4, 32), num_channels=in_ch, eps=1e-6) # 将channel（128）切分成许多组（32）进行归一化
        self.up = up
        self.down = down
        self.fir = fir
        self.fir_kernel = fir_kernel
        ...
        self.GroupNorm_1 = nn.GroupNorm(num_groups=min(
            out_ch // 4, 32), num_channels=out_ch, eps=1e-6)
        self.Dropout_0 = nn.Dropout(dropout) # 这里没有起到dropout的作用
        self.Conv_1 = conv3x3(out_ch, out_ch, init_scale=init_scale)
